...
Add an expanded permission handling API for WebView2, WKWebView, webkit6, and Android.
This includes:
...
- Linux (webkit6): `ClipboardRead` via `ClipboardPermissionRequest`; `MediaKeySystemAccess` via `MediaKeySystemPermissionRequest`; `getDisplayMedia` handled natively by webkit6 (no custom interception needed).
