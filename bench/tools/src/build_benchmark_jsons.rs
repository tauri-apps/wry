...
fn main() {
   ...
   // current data
   let current_data_buffer = BufReader::new(File::open(target_dir().join("bench.json")).expect("Unable to read current data file"));
   let current_data: BenchResult = serde_json::from_reader(current_data_buffer).expect("Unable to read current data buffer");

   // all data's
   let all_data_buffer = BufReader::new(File::open(&wry_data).expect("Unable to read all data file"));
   let mut all_data: Vec<BenchResult> = serde_json::from_reader(all_data_buffer).expect("Unable to read all data buffer");
   ...
   write_json(&wry_data, &serde_json::to_value(&all_data).expect("Unable to build final json (alls)"));
   write_json(&wry_recent, &serde_json::to_value(&recent).expect("Unable to build final json (recent)"));
}
